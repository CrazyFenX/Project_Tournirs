...
namespace DataViewer_D_v._001
{
    public partial class secretaryForm : Form
    {
        ...
        public secretaryForm(secretaryMainForm secretaryMainForm)
        {
            ...
            this.Path_textBox.Text = secretaryMainForm.Path_textBox.Text;
        ...
        private void CreateGroup_button_Click(object sender, EventArgs e)
        {
            if (Path_textBox.Text != "")
            {
                ...
                foreach (Branch branchItem in tournir.Branches)
                    branchesComboBox.Items.Add(branchItem.ToString());

                branchNumTextBox.Text = Convert.ToString(tournir.Branches.Count + 1);
        ...
        private void CreateGroupSecond_button_Click(object sender, EventArgs e)
        {
            ...
            if (NumberOfGroup_textBox.Text != "" && categoryNameTextBox.Text != "" && branchesComboBox.SelectedIndex != -1)
            {
                ...
                group_new.name = categoryNameTextBox.Text.Replace(" ", "_");
                group_new.branchNumber = (ushort)(branchesComboBox.SelectedIndex + 1);
                ...
                if (Path_textBox.Text != "")
                {
                    try
                    {
                        ...
                        group_new.system = systemComboBox.SelectedIndex;
                        string testStr1 = "";
                        ...
                        try
                        {
                            ...
                            SecretaryController.insertGroup(group_new, testStr1, testStr, folderName);
                            tournir.groups.Add(group_new);

                            string folderName1 = folderName;
                            folderName1 = folderName1.Substring(0, folderName1.Length - tournir.name.Replace(" ","").Length - 4);

                            DirectoryInfo dirInfo = new DirectoryInfo(folderName1 + "DuetNumbers");
                            if (dirInfo.Exists)
                            {
                                dirInfo.CreateSubdirectory(group_new.number.ToString() + ") " + group_new.name);
                            }

                            dirInfo = new DirectoryInfo(folderName1 + "MarkBlanks");
                            if (dirInfo.Exists)
                                dirInfo.CreateSubdirectory(group_new.number.ToString() + ") " + group_new.name);

                            //dirInfo = new DirectoryInfo(folderName1 + "Results");
                            //if (dirInfo.Exists)
                            //    dirInfo.CreateSubdirectory(group_new.name);

                            dirInfo = new DirectoryInfo(folderName1 + "Diploms");
                            if (dirInfo.Exists)
                            {
                                dirInfo.CreateSubdirectory(group_new.number.ToString() + ") " + group_new.name);
                                dirInfo.CreateSubdirectory(group_new.number.ToString() + ") " + group_new.name + " scating");
                            }
                        }
                        catch (Exception ex)
                        {
            ...
            if (Path_textBox.Text != "")
                if (tournir.groups.Count != 0)
                    if (tournir.judges.Count != 0)
                    {
                        ...
                        MessageBox.Show("Не зарегистрировано ни одного судьи!");
                else
                    MessageBox.Show("Не зарегистрированно ни одной группы!");
            else
        ...
        private void newBranchButton_Click(object sender, EventArgs e)
        {
            if (!NewBranchClicked)
            {
                newBranchGroupBox.Visible = true;
                NewBranchClicked = true;
                newBranchButton.Text = "▲";
            }
            else
            {
                newBranchGroupBox.Visible = false;
                NewBranchClicked = false;
                newBranchButton.Text = "+";
            }
        }

        private void addBrancheButton_Click(object sender, EventArgs e)
        {
            if (HourBranchStartComboBox.SelectedIndex == -1 || MinutesBranchStartComboBox.SelectedIndex == -1 || HourBranchFinishComboBox.SelectedIndex == -1 || MinutesBranchFinishComboBox.SelectedIndex == -1 || branchNumTextBox.Text == "")
            {
                MessageBox.Show("Не все поля заполнены!");
            }
            else
            {
                if (HourBranchStartComboBox.SelectedIndex > HourBranchFinishComboBox.SelectedIndex)
                    MessageBox.Show("Неверное время!");
                else
                {
                    if (HourBranchStartComboBox.SelectedIndex == HourBranchFinishComboBox.SelectedIndex && MinutesBranchStartComboBox.SelectedIndex >= MinutesBranchFinishComboBox.SelectedIndex)
                        MessageBox.Show("Неверное время!"); 
                    else
                        try
                        {
                            Branch tmpBranch = new Branch(Convert.ToInt32(branchNumTextBox.Text), new TimeClass(HourBranchStartComboBox.SelectedIndex, MinutesBranchStartComboBox.SelectedIndex * 5), new TimeClass(HourBranchFinishComboBox.SelectedIndex, MinutesBranchFinishComboBox.SelectedIndex * 5));
                            SecretaryController.insertBranch(tmpBranch, Path_textBox.Text);
                            branchesComboBox.Items.Add(tmpBranch.ToString());
                            tournir.Branches.Add(tmpBranch);

                            branchNumTextBox.Text = Convert.ToString(tournir.Branches.Count + 1);
                            MessageBox.Show("Новое отделение успешно создано!");
                        }
                        catch (Exception ex)
                        {
        ...
        private void createPatternButton_Click(object sender, EventArgs e)
        {
            try
            {
                //MessageBox.Show(Path_textBox.Text.Remove(Path_textBox.Text.LastIndexOf('\\') + 1) + "Diploms\\pattern.docx");
                Word_Controller.CreateDiplomPattern(Path_textBox.Text.Remove(Path_textBox.Text.LastIndexOf('\\') + 1) + "Diploms\\pattern.docx");
                Word_Controller.CreateDiplomPattern(Path_textBox.Text.Remove(Path_textBox.Text.LastIndexOf('\\') + 1) + "Diploms\\patternScating.docx");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "", MessageBoxButtons.OK, MessageBoxIcon.Error);
